...
export default function InvestmentList({ investments, onDelete, onUpdate }: InvestmentListProps) {
  const getTypeColor = (type: Investment['type']) => {
    switch (type) {
      ...
      case 'fd': return 'bg-yellow-100 text-yellow-800';
  ...
  if (investments.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500">Get started by adding a new investment.</p>
      </div>
    );
  }
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {investments.map((investment) => (
            <tr key={investment.id}>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getTypeColor(investment.type)}`}>
